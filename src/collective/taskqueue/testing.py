...
class TaskQueueServerLayer(Layer):
    ...
    def setUp(self):
        ...
        xmlconfig.file(
            "configure.zcml", collective.taskqueue, context=self["configurationContext"]
        )
        ...
        if not self.zserver_enabled:
            self["server"] = TaskQueueServer(queue=self.queue, handler=logging_handler)
        else:
            self["server"] = TaskQueueServer(
                queue=self.queue, handler=zserver_handler, concurrent_limit=0
            )
...
TASK_QUEUE_ZSERVER_FIXTURE = LocalTaskQueueServerLayer(
    queue="test-queue", zserver_enabled=True
)
...
REDIS_TASK_QUEUE_ZSERVER_FIXTURE = RedisTaskQueueServerLayer(
    queue="test-queue", zserver_enabled=True
)
